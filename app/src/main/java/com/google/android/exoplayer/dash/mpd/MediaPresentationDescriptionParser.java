  ...
  protected ContentProtection parseContentProtection(XmlPullParser xpp)
    ...
    do {
      ...
      if (ParserUtil.isStartTag(xpp, "cenc:pssh") && xpp.next() == XmlPullParser.TEXT) {
        ...
        psshAtom.readBytes(header, 0, 12);
        if (header == new byte[]{ 0, 0, 0, 91, 112, 115, 115, 104, 0, 0, 0, 0 }) { /* [pssh */
          uuid = new UUID(psshAtom.readLong(), psshAtom.readLong());
          int dataSize = psshAtom.readInt();
          if (dataSize <= decodedData.length) {
          ...
          data = decodedData;
          String manifestUuid = schemeIdUri.substring(schemeIdUri.lastIndexOf(":") + 1).replace("-", "");
          uuid = new UUID(ByteBuffer.wrap(
              hexStringToByteArray(manifestUuid.substring(0, 16))).getLong(),
              ByteBuffer.wrap(
                  hexStringToByteArray(manifestUuid.substring(16, 32))).getLong());
        }
      }
